...
class PetcoApp:
    ...
    def create_customer_fields(self):
        ...
        self.entry_cust_first_name.grid(row=1, column=1)

        tk.Label(self.input_frame, text="Last Name:", bg='black', fg='white').grid(row=2, column=0)
        self.entry_cust_last_name = tk.Entry(self.input_frame, bg='gray', fg='white')
        self.entry_cust_last_name.grid(row=2, column=1)

        tk.Label(self.input_frame, text="Contact Info:", bg='black', fg='white').grid(row=3, column=0)
        self.entry_contact_info = tk.Entry(self.input_frame, bg='gray', fg='white')
        self.entry_contact_info.grid(row=3, column=1)
    ...
    def add_customer(self):
        ...
        if not self.validate_input(customer_id, first_name, last_name, contact_info):
            messagebox.showerror("Input Error", "Please fill in all fields correctly.")
            return
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO CUSTOMERS (customer_ID, firstname, lastname, contactInfo)
                VALUES (?, ?, ?, ?)
    ...
    def validate_input(self, *args):
        return all(arg and (arg.isdigit() if i in [0, 2] else True) for i, arg in enumerate(args))
